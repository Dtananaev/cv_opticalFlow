...
CTensor<float> Horn_SchunkOptFlow(CMatrix<float> image1, CMatrix<float> image2, int sigma, bool presmoothing, float alpha, float treshold){
...
//Jacoby method
        ...
        u_k_new(x,y)= ( ( - (Ix(x,y) * Iz(x,y)) / alpha ) - ( ( Ix(x,y)*Iy(x,y)*v_k(x,y) ) / alpha ) + ( u_k(x+1,y) + u_k(x-1,y) + u_k(x,y+1) + u_k(x,y-1) ) ) / (  4+ (Ix(x,y)*Ix(x,y) )/alpha  );

        v_k_new(x,y) = ( ( - (Iy(x,y) * Iz(x,y)) / alpha ) - ( ( Ix(x,y)*Iy(x,y)*u_k(x,y) ) / alpha ) + ( v_k(x+1,y) + v_k(x-1,y) + v_k(x,y+1) + v_k(x,y-1) ) ) / (  4+ (Iy(x,y)*Iy(x,y) )/alpha   );
...
int main(int argc, char** argv) {  
   ...
   seq = loadSequence("resources/gsalesman/t.txt");
    ...
    for (int i = 0; i < seq.size()-1; ++i){
      ...
      float alpha = 400;
      float treshold= 0.00000000000001;

        opticFlow=Horn_SchunkOptFlow(img1 ,img2, sigma,  presmoothing, alpha,  treshold);
